...
class Curriculum():
    def __init__(self, win_cond=0.6, success_percent=0.6, fail_percent=0.2, min_length=100):
...
def ppo_feature(**kwargs):
    ...
    config.task_fn = lambda: AdaTask('AllTenTorsionSet-v0', num_envs=config.num_workers, seed=random.randint(0,1e5), single_process=single_process)
    config.optimizer_fn = lambda params: torch.optim.RMSprop(params, lr=lr, alpha=0.99, eps=1e-5)
    config.network = model
