...
class ActorBatchNet(torch.nn.Module):
    def __init__(self, action_dim, dim, edge_dim=1, num_features=3):
        ...
        self.set2set = gnn.Set2Set(dim, processing_steps=6)
        self.bbset2set = gnn.Set2Set(dim, processing_steps=6)
    ...
    def forward(self, obs, states=None):
        ...
        bbouts = []
        for i in range(4):
            bbout = F.relu(self.lin0(bbdata[i].x))
            bbouts.append(bbout)
            bbh = bbout.unsqueeze(0)
            bbhs.append(bbh)
        ...
        for i in range(6):
            ...
            for j in range(4):
                bbm = F.relu(self.conv(bbouts[j], bbdata[j].edge_index, bbdata[j].edge_attr))
                bbouts[j], bbhs[j] = self.gru(bbm.unsqueeze(0), bbhs[j])
                bbouts[j] = bbouts[j].squeeze(0)
        ...
        bbpools = []
        for i in range(4):
            bbpools.append(self.bbset2set(bbouts[i], bbdata[i].batch))
