...
class ActorGAT(torch.nn.Module):
    def __init__(self, action_dim, dim):
        ...
        num_features = 3
        self.conv1 = gnn.GATConv(num_features, dim, heads=2)
        self.conv2 = gnn.GATConv(dim * 2, dim, heads=2)
        self.conv3 = gnn.GATConv(dim * 2, dim, heads=2)
        self.conv4 = gnn.GATConv(dim * 2, dim, heads=2)
        self.conv5 = gnn.GATConv(dim * 2, dim)
    ...
    def forward(self, obs, states=None):
        ...
        out = F.elu(self.conv1(data.x, data.edge_index))
        out = F.elu(self.conv2(out, data.edge_index))
        out = F.elu(self.conv3(out, data.edge_index))
        out = F.elu(self.conv4(out, data.edge_index))
        out = F.elu(self.conv5(out, data.edge_index))
...
class GATBatch(torch.nn.Module):
    def __init__(self, action_dim, dim, point_dim=3):
        ...
        self.actor = ActorGAT(action_dim, dim)
        self.critic = CriticGAT(action_dim, dim)
