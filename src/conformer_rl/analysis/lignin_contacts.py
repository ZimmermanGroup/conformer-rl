...
def setup_dataframe():
    ...
    display(df.sort_values(by=[DISTANCE_RATIO]))
    return mol

FUNC_GROUP_ID_1, ATOM_ID_1 = 'func_group_id_1', 'atom_id_1'
FUNC_GROUP_ID_2, ATOM_ID_2 = 'func_group_id_2', 'atom_id_2'
def num_contacts_per_conf(mol, smarts_1, smarts_2, matches, thresh_3d=4, thresh_topological=5):
    dist_matrix_2d, dist_matrices_3d = setup_dist_matrices()
...
def contacts():
    ...
    df[NUM_CONTACTS_PER_CONF] = conf_id_index.map(partial(func, matches=matches))
    NUM_POTENTIAL_CONTACTS_PER_CONF = 'num potential contacts per conf'
    df[NUM_POTENTIAL_CONTACTS_PER_CONF] = conf_id_index.map(partial(func, matches=matches, thresh_3d=1000))
