...
import stk
from conformer_rl.analysis.lignin_contacts import FUNC_GROUP_ID_1, setup_dist_matrices, setup_mol
...
# display(np.array(MMFFOptimizeMoleculeConfs(Chem.rdchem.Mol(mol), maxIters=0)))
...
(maccoll_distances,) = xr.broadcast(maccoll_distances)
maccoll_distances.coords[FUNC_GROUP_ID_1] = range(maccoll_distances.sizes[FUNC_GROUP_ID_1])
display(maccoll_distances)
maccoll_dataset = hv.Dataset(maccoll_distances)
display(maccoll_distances)
points = maccoll_dataset.to(hv.Points, kdims=['func_group_id_1', 'Lignin Maccoll mechanism distances'])
# points = hv.Points(df, kdims=['func_group_id_1', 'Lignin Maccoll mechanism distances'])
points.opts(
