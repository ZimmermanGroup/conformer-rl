from conformer_rl.molecule_generation import generate_molecule


def test_alkanes(mocker):
    cfg1 = generate_molecule.branched_alkane(10)
    assert cfg1.mol.GetNumAtoms() == 32
    cfg2 = generate_molecule.straight_alkane(12)
    assert cfg2.mol.GetNumAtoms() == 38
    cfg3 = generate_molecule.xorgate(2, 4)
...
def test_custom(mocker):
    ...
    cfg1 = generate_molecule.test_alkane()
    assert cfg1.tau == 503
    assert cfg1.E0 == 7.668625034772399

    cfg2 = generate_molecule.mol_from_molFile('filename')
...
def test_custom2(mocker):
    ...
    cfg2 = generate_molecule.mol_from_smiles('smiles')
